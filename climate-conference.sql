...
QUERIES:
1. Which VIPs are delivering a lecture or session, and which other VIPs are attending or receiving feedback from them?
2. Which VIPs have confirmed attendance for both the welcome party and the thank-you dinner?
3. Which hotels have been booked by each VIP for the conference?
4. Which VIPs are conducting or presenting events of a specific event type (for example, education or government)?
5. Find the number of VIPs attending each event, ordered by event attendance.
6. Find the number of VIPs associated with each event, ordered by event count.

ANSWERS:
-- 1. VIP presenters and the VIPs who receive feedback from them
SELECT 
    V.first_name || ' ' || V.last_name AS Presenter,
    V1.first_name || ' ' || V1.last_name AS Attendee
FROM vips V
LEFT JOIN vips V1
    ON V.member_id = V1.provides_feedback_to;
-- 2. VIPs attending both welcome party and dinner
SELECT 
    V.first_name || ' ' || V.last_name AS VIP_Name
FROM vips V
JOIN reservations R
    ON V.member_id = R.member_id
WHERE R.welcome_rsvp = 1
  AND R.dinner_rsvp = 1;
-- 3. Hotel booked by each VIP
SELECT 
    V.first_name || ' ' || V.last_name AS VIP_Name,
    H.hotel_name
FROM vips V
...
-- 4. VIP presenters by association type (example: education)
SELECT 
    V.first_name || ' ' || V.last_name AS VIP_Name,
    E.event_name,
    V.assoc_type
...
-- 5. Number of VIPs attending each event 
SELECT 
    E.event_name,
...
-- 6. Number of VIPs associated with each event 
SELECT 
    E.event_name,
    COUNT(V.member_id) AS total_vips
FROM vips V
